...
def my_shuffle(input_image, ratio):
    shape = input_image.shape
...
def prepare_data():
    ...
    hr_start_idx = params['ratio'] * params['edge'] / 2
    hr_end_idx = hr_start_idx + (params['lr_size'] - params['edge']) * params['ratio']
    ...
    for dir_idx, image_dir in enumerate(image_dirs):
        ...
        for root, dirnames, filenames in os.walk(image_dir + "/lr"):
            for filename in filenames:
                ...
                lr_data = lr_image_y.reshape((params['lr_size'] * params['lr_size'] * 3))
                sub_hr_image_y = hr_image_y[hr_start_idx:hr_end_idx:1,hr_start_idx:hr_end_idx:1]
                hr_data = my_anti_shuffle(sub_hr_image_y, params['ratio']).reshape(sub_hr_size * sub_hr_size * 3)
